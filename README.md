...
To evaluate the registration, the script also generates two text files (Results\DicesMRI.txt and Results\DicesCT.txt), the first file with the dice overlap between the registered CT manual segmentations and MRI segmentation and the second with the overlap between the registered manual MRI segmentation and CT segmentations of the circle of Willis. The original segmentations for CT and MRI are located in the folder cow_seg_labelsTr, the registered segmentation are in the Results folder by the name SegmentationCTtoMRI.nii.gz and SegmentationMRItoCT.nii.gz.
